...
export default class Thread implements ILazy {
  ...
  private parseRating(data: TJsonLD): TRating {
    ...
    const rating: TRating = {
      average: ratingTree["ratingValue"] ? parseFloat(ratingTree["ratingValue"] as string) : 0,
      best: ratingTree["bestRating"] ? parseInt(ratingTree["bestRating"] as string, 10) : 0,
      count: ratingTree["ratingCount"] ? parseInt(ratingTree["ratingCount"] as string, 10) : 0
    };
   ...
   */
  private cleanHeadline(headline: string | undefined | null): string {
    // Return empty string if headline is undefined or null
    if (!headline) return 'MARICON';
  ...
  private async elaborateResponse(html: string) {
    ...
    // Safely get dates with null checks
    const published = JSONLD["datePublished"] ? getDateFromString(JSONLD["datePublished"] as string) : undefined;
    const modified = JSONLD["dateModified"] ? getDateFromString(JSONLD["dateModified"] as string) : undefined;
    ...
    this._title = this.cleanHeadline(JSONLD["name"] as string);
    ...
    // Ensure articleSection exists before using toString()
    const section = JSONLD["articleSection"] ? JSONLD["articleSection"].toString().toLowerCase() : "";
